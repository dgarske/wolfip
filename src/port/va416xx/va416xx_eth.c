...
/*   - TX: OWN in des0 (doorbell); FS/LS/IC/TCH/size in des1 (control)     */
/*   - RX: OWN/FL/status in des0; RER/size in des1                          */
 ...
 * Normal (non-enhanced) DWC GMAC descriptor format:
 * TDES0 contains ONLY the OWN bit (bit 31) for CPU→DMA handoff.
 * All other TDES0 bits are TX STATUS written back by DMA on completion
 * (error flags, CC, UF, etc.).  They must NOT be pre-loaded by the CPU.
 *
 * NOTE: The "ALTDESCRIPTOR" (enhanced) format puts FS/LS/IC/TCH/TER
 * in TDES0 instead.  This hardware uses the normal format — confirmed
 * by observing that setting those bits in TDES0 causes the DMA to
 * advance linearly (ignoring des3) and never transmit any frames. */
#define TDES0_OWN       (1U << 31)  /* DMA owns descriptor (doorbell) */
/* Bits [30:0] are TX status written back by DMA — do not set on TX submit */

/* --- TX Descriptor TDES1 bits (Control, normal descriptor format) ---
 * In the normal DWC GMAC descriptor format, TX frame control belongs
 * in TDES1 (dmamac_cntl).  This matches the u-boot designware.c driver
 * (non-ALTDESCRIPTOR path). */
#define TDES1_IC        (1U << 31)  /* Interrupt on Completion */
#define TDES1_LS        (1U << 30)  /* Last Segment */
#define TDES1_FS        (1U << 29)  /* First Segment */
#define TDES1_DC        (1U << 28)  /* Disable CRC */
#define TDES1_DP        (1U << 27)  /* Disable Padding */
#define TDES1_TER       (1U << 25)  /* Transmit End of Ring */
#define TDES1_TCH       (1U << 24)  /* Second Address Chained (chain mode) */
#define TDES1_TBS2_SHIFT 11
#define TDES1_TBS2_MASK  0x003FF800U /* Buffer 2 Size [21:11] */
#define TDES1_TBS1_MASK  0x000007FFU /* Buffer 1 Size [10:0] */
...
static void eth_config_speed_duplex(void)
{
    ...
    maccr = ETH_MAC_CONFIG_PS_Msk |
            ...
            ETH_MAC_CONFIG_ACS_Msk |
            ETH_MAC_CONFIG_IPC_Msk |
            ETH_MAC_CONFIG_BE_Msk;  /* Frame Burst Enable — u-boot reference always sets this */
    if (speed_100)
...
static void eth_start(void)
{
    ...
    /* Settling delay after TE/RE assertion (~100µs).
     * Gives the TX/RX FIFO controllers time to initialize before DMA starts. */
    { volatile uint32_t _d; for (_d = 0; _d < 10000U; _d++) { } }

    /* Flush TX FIFO (FTF) BEFORE starting DMA.
     * u-boot DWC GMAC reference driver always applies FTF before ST.
     * FTF is self-clearing (~8 AHB cycles).  Must be applied while ST=0:
     * if applied after ST=1, it latches and flushes DMA-written data. */
    VOR_ETH->DMA_OPER_MODE |= ETH_DMA_OPER_MODE_FTF_Msk;
    {
        uint32_t _ftf;
        for (_ftf = 0; _ftf < 100000U; _ftf++) {
            if (!(VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_FTF_Msk))
                break;
        }
        ETH_DEBUG("  FTF flush: cleared=%lu (t=%lu)\n",
                  (unsigned long)!(VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_FTF_Msk),
                  (unsigned long)_ftf);
    }
    ...
    ETH_DEBUG("  DMA_OPER_MODE=0x%08lX (FTF=%lu TSF=%lu TTC=%lu ST=%lu)\n",
              (unsigned long)VOR_ETH->DMA_OPER_MODE,
              (unsigned long)!!(VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_FTF_Msk),
              (unsigned long)!!(VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_TSF_Msk),
              (unsigned long)((VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_TTC_Msk) >> ETH_DMA_OPER_MODE_TTC_Pos),
              (unsigned long)!!(VOR_ETH->DMA_OPER_MODE & ETH_DMA_OPER_MODE_ST_Msk));
...
static int eth_phy_init(void)
{
     ...
     *
     * Advertise only 10BASE-T speeds (10M FD + 10M HD).
     *
     * The FES bit in MAC_CONFIG is read-only=0 on this VA416xx silicon
     * variant — the MAC is permanently at 10Mbps internal timing and
     * expects TXCLK = 2.5MHz from the PHY.  If we advertise 100M and AN
     * negotiates 100M, the PHY outputs 25MHz TXCLK which the MAC cannot
     * use: TX is silently broken (confirmed by GPIO clock scan and FES
     * read-only test).  Restricting AN to 10M ensures TXCLK = 2.5MHz. */
    {
        ...
        an_adv &= ~0x01E0U;  /* clear 100M-FD, 100M-HD, 10M-FD, 10M-HD */
        an_adv |=  0x0060U;  /* advertise only 10M-FD (bit6) + 10M-HD (bit5) */
...
static int eth_send(struct wolfIP_ll_dev *dev, void *frame, uint32_t len)
{
    ...
    /* Set des1: frame control + buffer size.
     * Normal DWC GMAC format: FS/LS/IC/TCH live in TDES1, not TDES0.
     * TCH=1: DMA follows des3 (chain pointer) to find the next descriptor.
     * des3 was set permanently in eth_init_desc() and is never overwritten
     * by DMA writeback, so the ring wraps correctly.
     * Set des1 BEFORE des0 so the descriptor is complete when OWN is given. */
    desc->des1 = TDES1_IC | TDES1_LS | TDES1_FS |
                 TDES1_TCH | (dma_len & TDES1_TBS1_MASK);

    __DSB();

    /* Set des0: OWN only — this is the doorbell that transfers ownership to
     * the DMA.  All other TDES0 bits are TX status written back by DMA. */
    desc->des0 = TDES0_OWN;

    __DSB();

    /* Clear TU (Transmit Buffer Unavailable) if set, then kick TX DMA */
    if (VOR_ETH->DMA_STATUS & ETH_DMA_STATUS_TU_Msk)
        VOR_ETH->DMA_STATUS = ETH_DMA_STATUS_TU_Msk;
    VOR_ETH->DMA_TX_POLL_DEMAND = 0;

#ifdef DEBUG_ETH
    /* Per-TX diagnostic: sample MAC_DEBUG at 3 time points to verify
     * data actually enters the TX FIFO (TXFSTS≠0 at 10µs → MAC transmitted). */
    if (tx_pkt_count <= 5U) {
        volatile uint32_t _d;
        uint32_t mac_dbg_before, mac_dbg_imm, mac_dbg_10us, mac_dbg_500us;
        uint32_t dma_st, des0_wb, cur_desc;

        mac_dbg_before = VOR_ETH->MAC_DEBUG;
        ...
        ETH_DEBUG("  TX#%lu des1=0x%08lX des0_wb=0x%08lX curr=0x%08lX TS=%lu\n",
                  (unsigned long)tx_pkt_count,
                  (unsigned long)desc->des1, (unsigned long)des0_wb,
                  (unsigned long)cur_desc,
                  (unsigned long)((dma_st >> 20) & 0x7U));
        ETH_DEBUG("  TXFSTS: bef=%lu imm=%lu @10us=%lu @500us=%lu "
                  "TRCSTS: @10us=%lu hw_tx=%lu\n",
                  ...
                  (unsigned long)VOR_ETH->TXFRAMECOUNT_GB);
        if (dma_st & (1U << 13))
            ETH_DEBUG("  *** FATAL BUS ERROR (FBI) ***\n");
    }
#endif /* DEBUG_ETH */
