...
START_TEST(test_sock_accept_ack_with_payload_completes_handshake)
{
    ...
    ck_assert_uint_eq(new_ts->sock.tcp.ack, 2 + sizeof(payload));
}
END_TEST
...
START_TEST(test_tcp_handshake_and_fin_close_wait)
{
    ...
    inject_tcp_segment(&s, TEST_PRIMARY_IF, remote_ip, local_ip, remote_port, local_port, 2, server_seq + 1, 0x01);
...
START_TEST(test_tcp_rto_cb_syn_sent_requeues_syn_and_arms_timer)
{
    ...
    ck_assert_uint_eq(seg->flags, 0x02);
